...
public:
   ...
   */
  double getVelocity(double currentMotorTicks,
  ...
  /**
   * \brief Gets the motor velocity put through a an exponential moving average
   * filter
   *
   * This value is the same as the estimator's output velocity, but without the
   * motor gearset translation.
   *
   * \return Velocity in RPM out of 3600
   */
...
private:
  ...
  std::shared_ptr<double>(&velocity_error)}, speedController(speedController) {
  */
...
public:
   ...
   */
  Motor(const uint8_t smart_port, const double gearing = 200,
   ...
   */
  void set_position_target_absolute(double new_position,
   ...
   *        Target speed to perform the movement at
   */
  void set_position_target_relative(double new_position,
                                    std::int32_t velocity_cap = 200);
  ...
  /**
   * \brief Sets the custom speed controller used by
  ...
  /**
   * \brief Gets the current error between the motor's actual velocity and the
   * velocity target
   *
  ...
  /**
   * \brief Gets the most recent time at which the motor has updated its tick
   * count
   *
   * This value is based on the motor's internal clock, and drifts from the
   * brain's system timer by approximately 1 part in 95.
   *
   * \return Milliseconds
   */
   ...
   *
   * Sets the value at the pointer provided to the motor timestamp in
   * milliseconds
   *
   * \return Ticks
   */
   ...
   *
   * \return True if the most recent raw velocity measurement is 0, otherwise
   * False
   */

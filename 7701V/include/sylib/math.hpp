...
public:
  ...
  /**
   * \brief Gets the number of median values to average when the sample size is
   * even
   *
   * \return Center size
   */
...
public:
  ...
  /**
   * \brief Calculates the slope between the last input value and the current
   * input value
   *
   * \param input
   *        The raw input value
...
public:
  ...
  /**
   * \brief Outputs an estimated voltage value for acheiving the desired RPM
   * based on the kV function supplied in the constructor
   *
   * \param rpm
...
public:
   ...
   *
   * \param error
   *        A pointer to a double containing the error value to use for
   * calculations
   *
   * \param motorGearing
   *        The output speed of the motor, in RPM, if the motor internally
   ...
   *        A flag that when true shifts the P controller to using kP2 as the kP
   * constant when error is less than <maxRange>
   *
   * \param kP2
   *        The secondary kP value that is used when maxRangeEnabled is on and
   * the error is within maxRange
   *
   * \param maxRange
   *        Error range from the target value that is used for switching between
   * kP and kP2
   */
  ProportionalController(double kP, std::shared_ptr<double> error,
                         double motorGearing = 200,
                         bool maxRangeEnabled = false, double kP2 = 0,
                         double maxRange = 0);
  ...
  /**
   * \brief Sets the error range from the target value that is used for
   * switching between kP and kP2
   *
   * \param range
   *        Max error value
   */
...
public:
   ...
   *        A flag that when true enables anti-windup, setting the output value
   * to 0 when error is more than <antiWindupRange>
   *
   * \param antiWindupRange
   *        Error range from the target value that is used for anti-windup
   */
  IntegralController(double kI, std::shared_ptr<double> error,
                     double motorGearing = 200, bool antiWindupEnabled = false,
...
public:
   ...
   *        is spinning at 3600 RPM. 200 is default for a green motor cartridge
   */
  DerivativeController(double kD, std::shared_ptr<double> error,
                       double motorGearing = 200);
...
struct SpeedControllerInfo {
  ...
  /**
   * \brief Whether or not to change the P controller gain within a certain
   * target range
   */
  ...
  /**
   * \brief Secondary kP value that the P controller uses when inside the target
   * range, if enabled
   */
  double kP2 = 0;

  /**
   * \brief Positive range from target above which the motor applies max voltage
   * outside of
   */
  double maxVoltageRange = 0;

  /**
   * \brief Whether or not to enable coast-down to more quickly move to a lower
   * velocity.
   */
  bool coastDownEnabled = false;

  /**
   * \brief Negative range from target below which the applied voltage is
   * multiplied by the coast-down modifier
   */
  double coastDownRange = 0;

  /**
   * \brief Constant to multiple the applied voltage by if error is below the
   * coast-down range
   */
  double coastDownModifier = 1;

  SpeedControllerInfo(
      kv_fn_t kV = [](double rpm) { return 0; }, double kP = 0, double kI = 0,
      double kD = 0, double kH = 0, bool antiWindupEnabled = false,
      double antiWindupRange = 0, bool pRangeEnabled = false, double pRange = 0,
      double kP2 = 0, double maxVoltageRange = 0, bool coastDownEnabled = false,
      double coastDownRange = 0, double coastDownModifier = 1)
      : kV(kV), kP(kP), kI(kI), kD(kD), kH(kH),
        antiWindupEnabled(antiWindupEnabled), antiWindupRange(antiWindupRange),
        pRangeEnabled(pRangeEnabled), pRange(pRange), kP2(kP2),
        maxVoltageRange(maxVoltageRange), coastDownEnabled(coastDownEnabled),

...
public:
  ...
  /**
   * \brief A flag that when set to false turns off all lights on all addrled
   *objects
   **/
  ...
  /**
   * \brief Sets the light control mode to MANUAL without changing the saved
   * buffer
   */
  void turn_on();
   ...
   *
   * \param speed
   *        A value from 1-100 describing the speed of the pulse. 1 is slow, 100
   * is fast
   *
   * \param start_pos
   ...
   *
   * \param end_pixel
   *        The pixel number to end the pulse on. If this unset it defaults to
   * the end of the strip
   */
  void pulse(std::uint32_t color, int pulse_width, int speed = 1,
             int start_pos = 0, bool reverse = false, int end_pixel = -1);
   ...
   *
   * \param fade_width
   *        A value describing how many pixels long to make the transition
   * between colors.
   *
   * \param reverse
   *        How many pixels down the strip to start
   ...
   *
   * \param hsv_mode
   *        A flag that enables the gradient to work in HSV space rather than
   * RGB
   */
  void gradient(std::uint32_t start_color, std::uint32_t end_color,
                int fade_width = 0, int start_pos = 0, bool reverse = false,
                bool hsv_mode = false);

  /**
   * \brief Automatically rotates the colors of a provided buffer through the
   * light strip periodically
   *
   * Sets the control mode to CYCLE
   *
   * \param color
   *        Buffer to rotate, use *<Addrled object name> to use the current
   * saved buffer
   *
   * \param speed
   *        A value from 1-100 describing the speed of the pulse. 1 is slow, 100
  ...
  /**
   * \brief Adds the provided red, green, and blue values to the colors sent to
   * the lights.
   *
   * Does not modify the saved buffer, can be used over other control modes
   *
   ...
   *
   * \return Value from 1-22 describing a smart port. 22 is the built-in three
   * wire ports.
   */
  int get_smart_port();
   ...
   *
   * \param step
   *        The number of steps of size (1/fade_width) from the start value
   * towards the end value
   *
   * \param fade_width
   ...
   * \return Hex code describing pixel color
   */
  static std::uint32_t interpolate_hsv(std::uint32_t start_color,
                                       std::uint32_t end_color, int step,
                                       int fade_width);
   ...
   * \return Hex code describing pixel color
   */
  static std::uint32_t interpolate_rgb(std::uint32_t start_color,
                                       std::uint32_t end_color, int step,
                                       int fade_width);

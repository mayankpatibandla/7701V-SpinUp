...
MedianFilter::MedianFilter(int sampleSize, int meanSizeEven, int meanSizeOdd)
    : sampleSize(sampleSize), meanSizeEven(meanSizeEven),
      meanSizeOdd(meanSizeOdd) {
  queueLength = 0;
  medianValue = 0;
}
...
RangeExtremaFilter::RangeExtremaFilter(int sampleSize)
...
double SympleDerivativeSolver::solveDerivative(double input) {
  ...
  deltaInputFunctionValue =
      currentInputFunctionValue - previousInputFunctionValue;
...
double SympleDerivativeSolver::getCurrentDerivative() const {
  return derivativeFunctionValue;
}

double SympleDerivativeSolver::getCurrentInputValue() const {
...
ProportionalController::ProportionalController(double kP,
                                               std::shared_ptr<double> error,
                                               double motorGearing,
                                               bool maxRangeEnabled, double kP2,
                                               double maxRange)
    : kP(kP), motorGearing(motorGearing), error(error), maxRange(maxRange),
      maxRangeEnabled(maxRangeEnabled), kP2(kP2) {
  proportional = 0;
}
...
void ProportionalController::setMaxRangeEnabled(bool enabled) {
  maxRangeEnabled = enabled;
}
...
IntegralController::IntegralController(double kI, std::shared_ptr<double> error,
    ...
    : kI(kI), motorGearing(motorGearing), error(error),
...
void IntegralController::setAntiWindupEnabled(bool enabled) {
  antiWindupEnabled = enabled;
}

void IntegralController::setAntiWindupRange(double range) {
  antiWindupRange = range;
}

DerivativeController::DerivativeController(double kD,
                                           std::shared_ptr<double> error,
                                           double motorGearing)
...
TakeBackHalfController::TakeBackHalfController(double kH,
                                               std::shared_ptr<double> error)
    : error(error), kH(kH) {
  output = 0;
  previousError = 0;

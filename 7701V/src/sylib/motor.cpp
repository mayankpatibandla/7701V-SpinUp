...
SylviesPogVelocityEstimator::SylviesPogVelocityEstimator(double motorGearing)
    : smaFilterVelocity(3), smaFilterAccel(15), smaFilterJerk(3),
      smaFilterSnap(3), medianFilter(7, 2, 1), emaFilter(),
      motorGearing(motorGearing), preFilterAccelSolver(), preFilterJerkSolver(),
      preFilterSnapSolver(), outputAccelSolver(), outputJerkSolver(),
      outputSnapSolver(), maxFilterAccel(20) {
...
double SylviesPogVelocityEstimator::getVelocity(
  ...
  dT = 5 * std::round((currentInternalMotorClock - previousInternalMotorClock) /
                      5.0);
  if (dT == 0) {
  ...
  preFilterAcceleration =
      preFilterAccelSolver.solveDerivative(medianFilteredVelocity);
  ...
  accelCalculatedkA =
      0.75 * (1 - (1 / ((maxFilteredAccel * maxFilteredAccel / 50) + 1.013)));

  emaFilteredVelocity =
      emaFilter.filter(smaFilteredVelocity, accelCalculatedkA);
...
double SylviesPogVelocityEstimator::getVelocityNoCalculations() const {
  return outputVelocity;
}

double SylviesPogVelocityEstimator::getRawVelocity() const {
  return rawVelocity;
}

double SylviesPogVelocityEstimator::getRawPosition() const {
  return oldMotorTicks;
}

double SylviesPogVelocityEstimator::getEmaFilteredVelocity() const {
  return emaFilteredVelocity;
}

double SylviesPogVelocityEstimator::getSmaFilteredVelocity() const {
...
double SylviesPogVelocityEstimator::getAcceleration() const {
...
double SylviesPogVelocityEstimator::getCalculatedkA() const {
  return accelCalculatedkA;
}

double SylviesPogVelocityEstimator::getMaxFilteredAcceleration() const {
  return maxFilteredAccel;
}

Motor::Motor(const uint8_t smart_port, const double gearing, const bool reverse,
             const SpeedControllerInfo speedController)
    : Device(5, 0), smart_port(smart_port), gearing(gearing), reversed(reverse),
      ...
      motorIController{speedController.kI,
                       std::shared_ptr<double>(&velocity_error), gearing, true,
                       speedController.antiWindupRange},
      motorDController{speedController.kD,
                       std::shared_ptr<double>(&velocity_error), gearing},
      motorTBHController{speedController.kH,
                         std::shared_ptr<double>(&velocity_error)},
  ...
  if (sylib_control_mode == SylibMotorControlModeOFF) {
    ...
    if (((velocity_target > 0)) &&
      ...
      voltage_target = 12000;
               ...
               (velocity_error <
                speedController.maxVoltageRange * gearing / 3600)) {
      voltage_target = -12000;
    } else {
      voltage_target = motorVoltageEstimator.estimate(velocity_target) +
                       *motorPController + *motorIController +
                       *motorDController + *motorTBHController;
    }

    if (speedController.coastDownEnabled) {
      if ((velocity_target > 0) &&
          velocity_error < -speedController.coastDownRange) {
        voltage_target *= speedController.coastDownModifier;
      } else if ((velocity_target < 0) &&
                 velocity_target > speedController.coastDownRange) {
        voltage_target *= speedController.coastDownModifier;
      }
    }
...
void Motor::set_position_target_absolute(double new_position,
                                         std::int32_t velocity_cap) {
  mutex_lock _lock(sylib_port_mutexes[smart_port - 1]);
...
void Motor::set_position_target_relative(double new_position,
                                         std::int32_t velocity_cap) {
  mutex_lock _lock(sylib_port_mutexes[smart_port - 1]);

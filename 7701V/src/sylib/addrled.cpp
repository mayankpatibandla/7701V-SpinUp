...
namespace sylib {
Addrled::Addrled(const uint8_t smart_port, const uint8_t adi_port,
                 const uint8_t strip_length, std::vector<uint32_t> colors)
    : Device(6, 0), smart_port(smart_port), adi_port(adi_port),
      strip_length(strip_length), device(vexDeviceGetByIndex(smart_port - 1)) {
...
Addrled::~Addrled() {
  getAllAddrleds().erase(
      std::remove(getAllAddrleds().begin(), getAllAddrleds().end(), this));
}
...
const std::vector<uint32_t> Addrled::off_buffer =
    std::vector<uint32_t>(64, 0x000000);
...
void Addrled::update() {
  ...
  if (!addrled_enabled) {
    vexDeviceAdiAddrLedSet(device, adi_port - 1,
                           (std::uint32_t *)off_buffer.data(), 0, 64, 0);
    return;
  }
  ...
  if (addrledControlMode == SylibAddrledControlModeMANUAL) {
    ...
    lightOutput = buffer;
    for (int i = 0;
         i < ((int)((currentTime - cycleStartMovementTime) / controlSpeed -
                    cyclePixelsShifted));
         i++) {
  ...
  if (sendingPulse) {
    ...
    lightOutput = buffer;
    for (int i = 0;
         i < ((int)((currentTime - pulseStartMovementTime) / pulseSpeed -
                    pulsePixelsShifted));
         i++) {
  ...
  vexDeviceAdiAddrLedSet(device, adi_port - 1,
                         (std::uint32_t *)lightOutput.data(), 0, strip_length,
                         0);
}
...
void Addrled::pulse(std::uint32_t color, int pulse_width, int speed,
                    int start_pos, bool reverse, int end_pixel) {
  mutex_lock _lock(sylib_port_mutexes[smart_port - 1]);
  sendingPulse = true;
  if (speed > 100) {
    speed = 100;
...
std::uint32_t Addrled::interpolate_rgb(std::uint32_t start_color,
...
std::uint32_t Addrled::interpolate_hsv(std::uint32_t start_color,
                                       std::uint32_t end_color, int step,
                                       int fade_width) {
...
void Addrled::gradient(std::uint32_t start_color, std::uint32_t end_color,
                       int fade_width, int start_pos, bool reverse,
                       bool hsv_mode) {
  mutex_lock _lock(sylib_port_mutexes[smart_port - 1]);
  if (fade_width == 0) {
  ...
  for (int i = 0; i < fade_width; i++) {
    if (hsv_mode) {
      buffer[start_pos + i] =
          interpolate_hsv(start_color, end_color, i, fade_width);
    } else {
      buffer[start_pos + i] =
          interpolate_rgb(start_color, end_color, i, fade_width);
    }
  }
  addrledControlMode = SylibAddrledControlModeMANUAL;
}

void Addrled::cycle(std::vector<uint32_t> pattern, int speed, int start_pos,
                    bool reverse) {
  mutex_lock _lock(sylib_port_mutexes[smart_port - 1]);

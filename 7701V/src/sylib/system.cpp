...
void SylibDaemon::startSylibDaemon() {
  ...
  if (!daemonStarted) {
    managerTask =
        std::unique_ptr<sylib::Task>(new sylib::Task(managerTaskFunction));
#ifdef SYLIB_ENV_PROS
...
void SylibDaemon::removeSubTask(sylib::Device *objectPointerToSchedule) {
  mutex_lock _lock(mutex);
  getLivingSubtasks().erase(std::remove(getLivingSubtasks().begin(),
                                        getLivingSubtasks().end(),
                                        objectPointerToSchedule));
}

void SylibDaemon::removeSubTaskByID(int idToKill) {
  mutex_lock _lock(mutex);
  getLivingSubtasks().erase(std::remove_if(
      getLivingSubtasks().begin(), getLivingSubtasks().end(),
...
[[noreturn]] int SylibDaemon::managerTaskFunction() {
  ...
  */
  ...
  // alternating. Over 3ms, the total length of time in micros should be either
  // around 3040 or 960 Daemon needs to start on a cycle to be directly opposite
  // of vexBackgroundProcessing() vexBackgroundProcessing always runs after a
  // short cycle, meaning the sylib daemon needs to start after a long cycle
  // Values offset by 20 to give room for error, the groupings are very tight so
  // it shouldnt matter
  ...
  while (1) {
    {
      ...
      for (auto &subTask : getLivingSubtasks()) {
        if (!subTask->getSubTaskPaused() &&
            ((frameCount + subTask->getUpdateOffset()) %
                 subTask->getUpdateFrequency() ==
             0)) {
          subTask->update();
        }
      }
      if (frameCount % 5 == 0) {
        ...
        if (vexControllerGet(kControllerMaster,
                             static_cast<V5_ControllerIndex>(18))) {
...
Device::Device(int interval, int offset)
    : updateFrequency(interval), updateOffset(offset) {
  startSubTask();
}
